...
namespace Sample
{
    public class KidsControlScript : MonoBehaviour
    {
        ...
        private static readonly int StandUpDiveState = Animator.StringToHash(
            "Base Layer.standup_dive"
        );
        ...
        private static readonly int FaintDamageState = Animator.StringToHash(
            "Base Layer.faint_damage"
        );
        private static readonly int StandUpFaintState = Animator.StringToHash(
            "Base Layer.standup_faint"
        );
        private static readonly int ShiftForwardState = Animator.StringToHash(
            "Base Layer.shift_forward"
        );
        ...
        private static readonly int StandUpCrouchState = Animator.StringToHash(
            "Base Layer.standup_crouch"
        );
        private static readonly int ShiftCrouchState = Animator.StringToHash(
            "Base Layer.shift_crouch"
        );
        private static readonly int StandUpCrawlState = Animator.StringToHash(
            "Base Layer.standup_crawl"
        );
        private static readonly int ShiftCrawlState = Animator.StringToHash(
            "Base Layer.shift_crawl"
        );
        private static readonly int HappyState = Animator.StringToHash("Base Layer.happy");
        private static readonly int FrustrationState = Animator.StringToHash(
            "Base Layer.frustration"
        );
        ...
        private static readonly int LookAroundState = Animator.StringToHash(
            "Base Layer.lookaround"
        );
        ...
        private static readonly int SlingshotChargeState = Animator.StringToHash(
            "Base Layer.slingshot_charge"
        );
        private static readonly int ShotState = Animator.StringToHash("Base Layer.shot");
        private static readonly int ShotChargeState = Animator.StringToHash(
            "Base Layer.shot_charge"
        );
        private static readonly int BookState = Animator.StringToHash("Base Layer.book");
        private static readonly int BookReaction1State = Animator.StringToHash(
            "Base Layer.book_reaction1"
        );
        private static readonly int BookReaction2State = Animator.StringToHash(
            "Base Layer.book_reaction2"
        );
        private static readonly int StickyState = Animator.StringToHash("Base Layer.sticky");
        private static readonly int DamageStickyState = Animator.StringToHash(
            "Base Layer.damage_sticky"
        );
        private static readonly int ReleaseStickyState = Animator.StringToHash(
            "Base Layer.release_sticky"
        );
        private static readonly int TrappedState = Animator.StringToHash("Base Layer.trapped");
        private static readonly int DamageTrappedState = Animator.StringToHash(
            "Base Layer.damage_trapped"
        );
        private static readonly int ReleaseTrappedState = Animator.StringToHash(
            "Base Layer.release_trapped"
        );
        private static readonly int TreatOwnState = Animator.StringToHash("Base Layer.treat_own");
        private static readonly int DamageTreatOwnState = Animator.StringToHash(
            "Base Layer.damage_treat_own"
        );
        ...
        private static readonly int DamageTreatedState = Animator.StringToHash(
            "Base Layer.damage_treated"
        );
        ...
        private static readonly int CaughtDownState = Animator.StringToHash(
            "Base Layer.caught_down"
        );
        private static readonly int CaughtStandUpState = Animator.StringToHash(
            "Base Layer.caught_standup"
        );
        ...
        void Start()
        {
            ...
            _EventObjMaterial = GameObject
                .Find("eventObj/EventObj")
        ...
        private void STATUS()
        {
            ...
            if (_Animator.GetFloat(PoseParameter) > 0 && _Animator.GetFloat(PoseParameter) <= 1)
            {
                ...
                _Animator.GetFloat(PoseParameter) == 0 && _Animator.GetFloat(PoseParameter) > 1
            )
            {
                _Status[Crouch] = false;
            }
            ...
            if (Input.GetKey(KeyCode.UpArrow))
            {
                ...
                if (_Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == MoveState)
                {
                    Vector3 velocity =
                        ...
                        * new Vector3(0, 0, speed * _Animator.GetFloat(StaminaParameter));
                    MOVE_XZ(velocity);
                    MOVE_RESET();
                }
            }
        ...
        //--------------------------------------------------------------------- Footstep
        // Animation Event Function (move_run)
        ...
        private void DIVE()
        {
            ...
            {
                if (
                    _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == DiveState
                    && !_Animator.IsInTransition(0)
                )
                {
        ...
        private void DAMAGE()
        {
            if (Input.GetKeyDown(KeyCode.Q))
            {
                ...
                DAMAGE_LV(damage);
                if (
                    _Animator.GetFloat(DamageParameter) < 2 && _Animator.GetFloat(PoseParameter) < 2
                )
                {
                    _Animator.CrossFade(DamageState, 0.1f, 0, 0);
                }
                else if (
                    _Animator.GetFloat(DamageParameter) == 2
                    && _Animator.GetFloat(PoseParameter) < 2
                )
                {
                    _Animator.CrossFade(DownState, 0.1f, 0, 0);
                }
                else if (
                    _Animator.GetFloat(DamageParameter) == 3
                    || _Animator.GetFloat(PoseParameter) == 2
                )
                {
        ...
        private void STAMINA()
        {
            ...
            if (
                _Animator.GetFloat(SpeedParameter) == 2
            ...
            )
            {
                ...
                _Animator.GetFloat(SpeedParameter) == 1
                || _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash != MoveState
            )
            {
        ...
        private void HAMMER()
        {
            ...
            if (
                _Animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 2.5f
        ...
        // Animation Event Function (action_smokebomb)
        ...
        private void BOOK()
        {
            ...
            {
                if (
                    _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == BookReaction1State
        ...
        private void ITEM_ACTION()
        {
            ...
            )
            {
                ...
                _ObjItem.gameObject.SetActive(true);
                _ObjItem.transform.position =
                    this.transform.position + this.transform.rotation * new Vector3(0, 0.25f, 0.5f);
                _Item1Icon.gameObject.SetActive(false);
            }
        }
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            if (other.gameObject.name == "HillEdge")
            {
                if (!CheckGrounded())
                {
                    ...
                    Vector3 pos = new Vector3(
                        _HillEdge.transform.position.x,
                        _HillEdge.transform.position.y,
                        _HillEdge.transform.position.z
                    );
            ...
            if (
                other.gameObject.name == "StickyPanel"
                && _Animator.GetFloat(DamageParameter) < 2
                && _Animator.GetFloat(PoseParameter) == 0
            )
            {
        ...
        void OnTriggerStay(Collider other)
        {
            ...
            if (CheckGrounded())
            {
                if (other.gameObject.name == "KickObj")
                {
                    ...
                    if (
                        _Animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f
                        && _Animator.GetCurrentAnimatorStateInfo(0).fullPathHash == KickState
                        && !_Animator.IsInTransition(0)
                    )
                    {
                        _Animator.CrossFade(IdleState, 0.1f, 0, 0);
                ...
                if (other.gameObject.name == "BoyNPC")
                {
                    ...
                    )
                    {
                        ...
                        _BoyNPC.transform.position =
                            _BoyNPC.transform.position + new Vector3(0, 1, 0);
                    }
                }
            ...
            )
            {
                ...
                {
                    distance[i] =
                        (pos1.x - pos2[i].x) * (pos1.x - pos2[i].x)
                        + (pos1.y - pos2[i].y) * (pos1.y - pos2[i].y)
                        + (pos1.z - pos2[i].z) * (pos1.z - pos2[i].z);
                }
